export type Mailbox = { object_id: string; tenant_id: string; name: string; email: string };
export type Signal = { topic: string; category: string; signal_score: number; audience_strength: string; signal_date: string };
export type Contact = { first_name: string; last_name: string; job_title: string; email: string; phone: string };
export type Lead = { id: string; run_id: string; company_id: string; company_name: string; website: string; tier: string; score: number; score_reasons: string[]; signals: Signal[]; contacts: Contact[] };
export type EmailStatus = "draft" | "ready" | "sending" | "sent" | "failed" | "unknown";
export type Draft = { id: string; run_id: string; lead_id: string; company_name: string; to_name: string; to_email: string; subject: string; body: string; programme: string; ready_to_send: boolean; approved_by: string | null; sent_by: string | null; sender_email: string | null; status: EmailStatus; version: number; sent_at: string | null; last_error: string | null; message_id: string | null };
...
export type Settings = { zoominfo_configured: boolean; microsoft_configured: boolean; mail_provider: string; login_required: boolean; llm_configured: boolean; llm_provider: string; daily_send_limit: number; default_country: string; sender_name: string; sender_email: string };
