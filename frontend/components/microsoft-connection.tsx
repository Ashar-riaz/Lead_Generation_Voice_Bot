...
type Props = { mailbox: Mailbox | null; configured: boolean; disabled?: boolean; onChange: () => Promise<void> };

export default function MicrosoftConnection({ mailbox, configured, disabled, onChange }: Props) {
  ...
  return <section className="panel connection-card">
    <div className="row between"><span className="icon-tile"><Mail size={23} /></span><span className={`badge ${mailbox ? "ready" : "draft"}`}>{mailbox ? "Connected" : "Not connected"}</span></div>
    <h2>Microsoft email</h2>
    <p>{mailbox ? `Send as ${mailbox.email}. Review and approve each message before sending.` : "Optional. Connect your Microsoft work mailbox when you are ready to send reviewed emails."}</p>
