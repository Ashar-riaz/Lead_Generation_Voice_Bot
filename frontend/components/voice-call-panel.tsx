...
type Call = { id: string; lead_id: string; to_number: string; from_number: string; contact_name: string;
  ...
  voice: string; transcript: Turn[]; created_at: string };
...
export default function VoiceCallPanel({ lead, mock }: { lead: Lead; mock: boolean }) {
  const firstContact = lead.contacts.find(contact => contact.phone);
  const [phone, setPhone] = useState(firstContact?.phone || "");
  ...
  return <section>
    ...
    {current && <>
      ...
      {!!current.transcript.length && <><h3 className="subheading">Conversation transcript</h3>{current.transcript.map(turn => <div className="activity-card" key={turn.turn}>{turn.heard && <p><strong>Contact:</strong> {turn.heard}</p>}<p><strong>WTD assistant:</strong> {turn.reply}</p></div>)}</>}
    </>}
